...
<template>
  <v-dialog class="w-full max-w-4xl" v-model="deleteModalState" activator=".dlt-button" transition="none"
    ...
    <div class="bg-white w-full rounded rounded-xl shadow-2xl flex flex-col">
      <div class="modal-header flex justify-center items-center py-6 px-7">
        <h2 class="text-xl text-center" data-cy="modal-delete-title">
          Apakah anda yakin ingin menghapus activity "<span class="font-bold">{{ this.deletedText }}</span>"?
        </h2>
      </div>
      <div class="modal-footer flex justify-center gap-4 items-center px-7 pb-6">
        <button data-cy="modal-delete-cancel-button"
        ...
        <button data-cy="modal-delete-confirm-button"
          class="text-xl bg-red-400 text-white font-semibold py-3 px-7 rounded-full" @click="deleteItem">
          Hapus
        </button>
      </div>
    </div>
